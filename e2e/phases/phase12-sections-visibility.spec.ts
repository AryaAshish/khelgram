...
test.describe('Phase 12: Section visibility CMS @phase12', () => {
  ...
  test('admin can hide About section on public homepage', async ({ page }) => {
    await loginAsAdmin(page)
    await page.getByRole('link', { name: 'Content' }).click()
    await page.getByRole('tab', { name: 'Sections' }).click()
    await page.getByLabel('Show About section').uncheck()
    ...
    await expect(page.getByRole('heading', { name: 'About Khelgram Foundation' })).toHaveCount(0)

    await loginAsAdmin(page)
    await page.getByRole('link', { name: 'Content' }).click()
    await page.getByRole('tab', { name: 'Sections' }).click()
    await page.getByLabel('Show About section').check()
    await page.getByRole('button', { name: 'Save section settings' }).click()
    await expect(page.getByText('Section visibility saved')).toBeVisible()
